...
def compute_portvals(  		  	   		 	   			  		 			     			  	 
    ...
    for index, row in portvals.iterrows():
        ...
        #        if order[0] in current_portfolio:
        if trade > 0 and symbol in current_portfolio:
            current_portfolio[symbol]['quantity'] += trade
            current_portfolio['cash']['value']+=get_value(commission, impact, get_closing_price(symbol, index, prices),
                              abs(trade), "BUY")
        if trade <0 and symbol in current_portfolio:
            current_portfolio[symbol]['quantity'] += trade
            current_portfolio['cash']['value'] += get_value(commission, impact,
                                                            get_closing_price(symbol, index, prices),
                                                            abs(trade), "SELL")

        if trade > 0 and symbol not in current_portfolio:
